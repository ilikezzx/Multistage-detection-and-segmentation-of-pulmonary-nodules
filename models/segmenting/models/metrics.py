...
class DiceCoefficient:
    ...
    def __call__(self, input, target):
        return torch.mean(compute_per_channel_dice(input, target, epsilon=self.eps))
...
class MeanIoU:
    ...
    def __call__(self, input, target):
       ...
       """
        ...
        for _input, _target in zip(input, target):
            binary_prediction = self._binarize_prediction(_input, n_classes)
            ...
            assert per_channel_iou, " All channel were ignore from computation"
    ...
    def _binarize_prediction(self, input, n_classes):
        """
        ...
        return torch.zeros_like(input, dtype=uint8).scatter_(0, max_index, 1)
