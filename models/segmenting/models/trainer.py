...
class UNet3DTrainer:
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion,
        ...
        # self.device = torch.device('cuda:1') # 必要时注释
        ...
        self.num_iterations = num_iterations    # 总共的迭代次数
        self.num_epoch = num_epoch  # 现在的轮次
    ...
    # 训练
    def fit(self):
        # 利用UNet模型参数  初始化部分模型参数
        # pretrained_model = torch.load(r'/content/drive/MyDrive/pytorch3dunet/CHECKPOINT_DIR_UNet_dice/best_checkpoint.pt')['model_state_dict']
        ...
        # print('*'*10,"Model更新成功!","*"*10)
        print('start lr=',self.optimizer.param_groups[0]['lr'])
        # for param_group in self.optimizer.param_groups:
        #     param_group['lr'] = 5e-4

        for _ in range(self.num_epoch, self.max_num_epochs):
            # train for one epoch
            print("now epoch=",self.num_epoch)
            if self.num_epoch in [50,95]:
                print('学习率缩小0.1倍!')
                for param_group in self.optimizer.param_groups:
    ...
    def train(self, train_loader):
        ...
        print('now lr=',self.optimizer.param_groups[0]['lr'])
        ...
        for i, t in enumerate(tqdm(train_loader)):
            ...
            input, target, weight = self._split_train_batch(t)
            ...
            if self.num_iterations % self.validate_after_iters == 0:
                ...
                # restart train mode
                self.model.train()
                # adjust learning rate if necessary 修改学习率
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                if hasattr(self.model.module, 'final_activation') and self.model.module.final_activation is not None:
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for i, t in enumerate(tqdm(val_loader)):
    ...
    def _log_params(self):
        ...
        for name, value in self.model.named_parameters():
            ...
            # self.writer.add_histogram(name + '/grad', value.weight.grad.data.cpu().numpy(), self.num_epoch)
